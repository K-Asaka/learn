...
### コードエディタを設定する
JBuilderでは、コード入力を支援する機能が満載されている。
プログラマーはその機能をさらに自分用にカスタマイズすることができる。
ツール→エディタの設定を選択して、表示されるエディタ設定ダイアログボックスを使って行う。


### Hello Java!アプリケーションの作成
実際の操作が行えないので省略。
...
### コマンドラインから実行する
Javaアプリケーションは、java.exeを使用して起動する。
このとき、クラスファイルへのパスをパラメータとして指定する必要がある。
JBuilderで作成したJavaアプリケーションのクラスファイルは、プロジェクトウィザードの2/3ページの出力パスで指定したディレクトリにある。
プロジェクトウィザードの2/3ページで何も変更していなければ、プロジェクトウィザードの1/3ページのディレクトリで指定したパス\classesにある。
そしてクラスファイルは、アプリケーションウィザードの1/2ページで指定した以下の形で指定する。
```
パッケージ.クラス
```
コマンドラインからは、次のようにコマンドを入力して実行する。
```
java -classpath C:\JAVA\SAMPLE\jb476\classes\jb476.Application1
```


### MS-DOSバッチファイルで実行する
コマンドを毎回入力して実行する手間を省くために、バッチファイルを作成して実行することができる。
メモ帳などのテキストエディタを使ってコマンドラインを記述し、拡張子を「.bat」にして作成する。
また、通常Javaアプリケーションの終了と同時にプロンプト画面は閉じられるが、万が一に備えてバッチファイルの最後にexitコマンドを入力しておけば完璧。
バッチファイルのアイコンをダブルクリックすると、作成したJavaアプリケーションが起動する。


### ショートカットを使って実行する
バッチファイルのようなファイルを作成することなく、直接java.exeにパラメータを渡して実行する。
作成の手順は次の通り。
1. ショートカットを作成するディレクトリで、右クリックで表示されるショートカットメニューから、新規作成→ショートカットを選択
2. ショートカットの作成ダイアログボックスにコマンドラインを入力して次へボタンを押す
3. 名前の指定のダイアログボックスには、任意のショートカット名を入力して完了ボタンをクリックする
ショートカットのアイコンをダブルクリックすると、作成したJavaアプリケーションが起動する。


### タブパネルを配置する
480
### デバッグを行う
### Hello Java!アプレットの作成手順
### アプレットプロジェクトの作成
### アプレットのひな形の作成
### レイアウトを設定する
### コンポーネントを追加する
