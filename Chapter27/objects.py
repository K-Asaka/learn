import pygame, config as config, os
...
class Weight(SquishSprite):
    ...
    def update(self):
        """
        ウェイトを垂直方向(下向き)に、スピードに対応する距離だけ移動させる。
        さらに、画面下端に達したかどうかに応じてlanded属性を設定する。
        """
        for wait_time in range(0, 100000 - self.speed): pass
...
class Banana(SquishSprite):
    ...
    def touches(self, other):
        ...
        Rectのcolliderectメソッドは使わず(上側と左右のパディングと
        Rectのinflateメソッドを使って)バナナの上側と左右の
        「空の」領域を含まない長方形を新たに計算する。
        """
