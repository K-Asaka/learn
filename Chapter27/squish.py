...
class Level(State):
    ...
    def __init__(self, number=1):
        ...
        speed += int((self.number-1) * config.speed_increase)
    ...
    def update(self, game):
        ...
        # このレベルのウェイトがすべてかわされたら、
        # ゲームをLevelClearedステートに切り換えるように指示する
    ...
    def display(self, screen):
        """
        最初の表示(画面全体の消去)後にステートを表示する。
        firstDisplayとは異なり、このメソッドは、
        self.sprites.drawから返される、更新が必要な部分を表す長方形の
...
class Paused(State):
    ...
    def handle(self, event):
        """
        イベントを処理する。
        具体的にはStateに委任する場合と(通常、終了の処理を行う)、
        キーの押下とマウスクリックに対応する場合がある。
        後者の場合はself.finishedを「真」に設定する。
        """
    ...
    def update(self, game):
        ...
        (self.finishedがtrue)、ゲームに対してself.next_state()
        (サブクラスで実装する)が表しているステートに
        切り換えるように指示する。
        """
        if self.finishied:
...
class Game:
    ...
    def run(self):
        ...
        # メインループ
        while True:
            # (1) nextStateが変更されている場合、新しいステートに移行して
            #     それを表示する(初回)
