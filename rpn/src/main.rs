...
fn rpn(exp: &str) -> f64 {
    ...
    // ;が付いていないので、この式が返した値を関数の戻り値として呼び出し元に返せる
...
where
    F: Fn(f64, f64) -> f64
/*
    <F>はこの関数がジェネリクスで、型パラメータとしてFを取ることを表す
    Fはwhere節で指定したトレイト境界を満たす型なら、どれにでもなれる。
    apply2関数では引数funの型は型パラメータFで、そのトレイト境界はFn(f64, f64) -> f64と
    している。これにより、rpn関数内で作った|x, y| x + yなどのクロージャが受け取れるようになる。
*/
{
